...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 15 01:28:47 2018
...
source_max_row = source_im.shape[0] - 1
source_max_col = source_im.shape[1] - 1
...
m = -source_max_col/source_max_row
select = -coords[1]-m*(source_max_row-coords[0]) >= 0
coords1 = coords[:, select]
coords2 = coords[:, ~select]

new_coords1 = transform(np.linalg.pinv(H1), coords1).astype(np.int)
new_coords2 = transform(np.linalg.pinv(H2), coords2).astype(np.int)
target_im = source_im
target_im[coords1[0], coords1[1], :] = source_im[new_coords1[0], new_coords1[1], :]
target_im[coords2[0], coords2[1], :] = source_im[new_coords2[0], new_coords2[1], :]
